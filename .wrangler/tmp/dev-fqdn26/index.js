...
// src/routes/page.ts
var formatBlockText = /* @__PURE__ */ __name((text) => {
  return text.replace(/\n/g, "<br />");
}, "formatBlockText");
async function pageRoute(req) {
  ...
  for (const blockId in allBlocks) {
    ...
    if (val?.properties?.title && Array.isArray(val.properties.title)) {
      val.properties.title = val.properties.title.map((textArr) => {
...
// .wrangler/tmp/bundle-QCf3mo/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-QCf3mo/middleware-loader.entry.ts
